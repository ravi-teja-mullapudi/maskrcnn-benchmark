...
def cat_pc(gt, predictions, thresholds):
    ...
    return recalls, precisions, ap, image_gts, gt_checked, predictions, fp

def compute_map_on_list(label_list, pred_all, accuracy_per_image):
    gt_list = []
    pred_list = []
    classes = {}
    image_ids = []
    ...
    for i, cat in enumerate(cat_list):
        if cat in cat_gt:
            r, p, ap, _, _, _ = cat_pc(cat_gt[cat], cat_pred[cat], thresholds)
            aps[:, i] = ap
            accuracy_dict[cat] = [ np.mean(ap*100), len(cat_gt[cat]) ]
    aps *= 100
    mAP = np.mean(aps)
    accuracy_dict['mAP'] = mAP

    assert(len(image_ids) == 1)
    accuracy_per_image[image_ids[0]] = accuracy_dict
...
if __name__ == "__main__":
    ...
    #                        'supervised.json')
    with open('pretrained.json') as pred_file:
        pred_all = json.load(pred_file)

    accuracy_per_image = {}

    for l in tqdm(bdd_labels_list):
        compute_map_on_list([l], pred_all, accuracy_per_image)

    with open('bdd_analysis_pretrained.json', 'w') as fp:
        json.dump(accuracy_per_image, fp)
